...
@section('panel')
    @php
        if(request()->routeIs('admin.deposit.*')) {
            $name = 'Deposit';
            $routeKey = 'deposit';
        } else {
            $name = 'Payment';
    ...
    <div class="row justify-content-center gy-4">
        <div class="col-xl-4 col-md-6">
            <div class="card overflow-hidden box--shadow1">
                <div class="card-body">
                    <h5 class="mb-20 text-muted">@lang("{$name} Via") @if($deposit->method_code < 5000) {{ __($deposit->gateway->name) }} @else @lang('Google Pay') @endif</h5>
                    <ul class="list-group">
        ...
        <div class="col-xl-8 col-md-6">
            <div class="card overflow-hidden box--shadow1">
                <div class="card-body">
                    <h5 class="card-title border-bottom pb-2">@lang("User {$name} Information")</h5>
                    @if($details != null)
                    ...
                    @if($deposit->status == Status::PAYMENT_PENDING)
                        <div class="row mt-4">
                            <div class="col-md-12">
                                ...
                                data-action="{{ route('admin.'.$routeKey.'.approve', $deposit->id) }}"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Reject :name Confirmation', ['name' => ucfirst($name)])</h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="las la-times"></i>
                    </button>
                </div>
                <form action="{{ route('admin.'.$routeKey.'.reject')}}" method="POST">
                    @csrf
                    <input type="hidden" name="id" value="{{ $deposit->id }}">
                    <div class="modal-body">
                        <p>@lang('Are you sure to') <span class="fw-bold">@lang('reject')</span> <span class="fw-bold text--success">{{ showAmount($deposit->amount)}}</span> @lang(":name of", ['name' => strtolower($name)]) <span class="fw-bold">{{ $deposit->user?->username }}</span>?</p>
